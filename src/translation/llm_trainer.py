...
ollama pull llama3.2:3b
    ...
    def pull_base_model(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(["ollama", "pull", self.model_name], capture_output=True, text=True, timeout=600)
    ...
    def extract_training_data(self) -> list[dict]:
        ...
        all_entries = list(
            self.db.dictionary_collection.find({"search_direction": {"$ne": "en_to_chk"}})  # Exclude reverse entries
        )
        ...
        for entry in all_entries:
            ...
            if len(chuukese_word) > 1 and len(english_translation) > 2:
                ...
                examples = [
                    {"input": f"Translate to English: {chuukese_word}", "output": english_translation},
                    {
                        "input": f"What does '{chuukese_word}' mean?",
                        "output": f"'{chuukese_word}' means {english_translation}",
                    },
                    {"input": f"Translate to Chuukese: {english_translation}", "output": chuukese_word},
                ]
    ...
    def create_modelfile(self, training_data: list[dict]) -> str:
        ...
        examples_text = "\n".join([f"Human: {ex['input']}\nAssistant: {ex['output']}" for ex in sample_examples])
                ...
                json={
                    ...
                    "options": {
                        ...
                        "stop": [
                            ...
                            "\n#",
                            "\nNote:",
                        ],  # Stop at common conversational patterns
                    },
                },
                timeout=30,
            )
