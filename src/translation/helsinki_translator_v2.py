...
class HelsinkiChuukeseTranslator:
    ...
    def __init__(
        self, base_model: str = "Helsinki-NLP/opus-mt-mul-en", reverse_model: str = "Helsinki-NLP/opus-mt-en-mul"
    ):
    ...
    def setup_models(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(self.local_chk_to_en) and os.path.exists(
                os.path.join(self.local_chk_to_en, "config.json")
            ):
            ...
            self.model = AutoModelForSeq2SeqLM.from_pretrained(
                model_path, dtype=torch.float16 if self.device == "cuda" else torch.float32
            ).to(self.device)

            # Create translation pipeline
            self.translator_pipeline = pipeline(
                "translation", model=self.model, tokenizer=self.tokenizer, device=0 if self.device == "cuda" else -1
            )

            # Reverse direction: English to Chuukese
            # Check for fine-tuned model first
            if os.path.exists(self.local_en_to_chk) and os.path.exists(
                os.path.join(self.local_en_to_chk, "config.json")
            ):
            ...
            self.reverse_model_obj = AutoModelForSeq2SeqLM.from_pretrained(
                reverse_model_path, dtype=torch.float16 if self.device == "cuda" else torch.float32
            ).to(self.device)
            ...
            if self.using_finetuned["chk_to_en"] or self.using_finetuned["en_to_chk"]:
                logger.info(
                    f"✅ Helsinki-NLP models loaded (fine-tuned: chk→en={self.using_finetuned['chk_to_en']}, en→chk={self.using_finetuned['en_to_chk']})"
                )
            else:
        ...
        try:
            ...
            for entry in all_entries:
                ...
                if chuukese_word and english_translation:
                    pair = TranslationPair(chuukese=chuukese_word, english=english_translation, source=source_text)
        ...
        try:
            ...
            def tokenize_function(examples):
                ...
                model_inputs = tokenizer(
                    inputs, max_length=128, truncation=True, padding="max_length", return_tensors=None
                )

                # Tokenize targets
                labels = tokenizer(
                    text_target=targets, max_length=128, truncation=True, padding="max_length", return_tensors=None
                )
    ...
    def load_fine_tuned_model(self, model_path: str, direction: str = "chuukese_to_english") -> bool:
        ...
        try:
            ...
            if direction == "chuukese_to_english":
                ...
                self.model = model
                self.translator_pipeline = pipeline(
                    "translation", model=model, tokenizer=tokenizer, device=0 if self.device == "cuda" else -1
                )
            else:
