...
class HelsinkiChuukeseTranslator:
        ...
        try:
            ...
            try:
                self.model = AutoModelForSeq2SeqLM.from_pretrained(
                    self.model_name, use_safetensors=True, trust_remote_code=False
                )
                ...
                self.model = AutoModelForSeq2SeqLM.from_pretrained(
                    self.model_name, dtype=torch.float32, trust_remote_code=False
                )
            ...
            try:
                self.reverse_model = AutoModelForSeq2SeqLM.from_pretrained(
                    self.reverse_model_name, use_safetensors=True, trust_remote_code=False
                )
            except Exception:
                self.reverse_model = AutoModelForSeq2SeqLM.from_pretrained(
                    self.reverse_model_name, dtype=torch.float32, trust_remote_code=False
                )
        ...
        try:
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    **inputs, max_length=128, num_beams=4, early_stopping=True, do_sample=False
                )
    ...
    def prepare_fine_tuning_data(self) -> Dataset:
        ...
        for src, tgt in zip(source_texts, target_texts):
            # Tokenize source
            src_tokens = self.tokenizer(src, max_length=128, padding="max_length", truncation=True, return_tensors="pt")

            # Tokenize target
            with self.tokenizer.as_target_tokenizer():
                tgt_tokens = self.tokenizer(
                    tgt, max_length=128, padding="max_length", truncation=True, return_tensors="pt"
                )
    ...
    def test_translation(self) -> dict[str, Any]:
        ...
        results = {"timestamp": datetime.now().isoformat(), "model": self.model_name, "tests": []}

        for chuukese_word in test_cases:
            try:
                translation = self.translate_chuukese_to_english(chuukese_word)
                results["tests"].append(
                    {"chuukese": chuukese_word, "english_translation": translation, "success": True}
                )
                logger.info(f"✅ {chuukese_word} → {translation}")
            except Exception as e:
                results["tests"].append({"chuukese": chuukese_word, "error": str(e), "success": False})
