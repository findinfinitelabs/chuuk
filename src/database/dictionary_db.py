...
class DictionaryDB:
    ...
    def add_dictionary_page(self, publication_id: str, filename: str, ocr_text: str, page_number: int = 1) -> str:
        ...
        self.pages_collection.update_one({"_id": result.inserted_id}, {"$set": {"entries_extracted": len(entries)}})

        return page_id

    def _extract_dictionary_entries(
        self, ocr_text: str, page_id: str, publication_id: str, filename: str, page_number: int = 1
    ) -> list[dict]:
        """
        ...
        for line_num, line in enumerate(lines):
            ...
            for pattern_num, pattern in enumerate(patterns, 1):
                ...
                if match:
                    ...
                    line_entries = self._extract_multiple_entries(
                        line,
                        pattern_num,
                        match,
                        page_id,
        ...
        for form_match in additional_forms:
            ...
            if self._validate_entry(word, full_definition):
                entry = self._build_entry(
                    ...
                    page_number,
                    line_number,
                    base_word or current_root_word,
                    is_base_word=False,
                )
    ...
    def _validate_entry(self, chuukese_word: str, english_translation: str) -> bool:
        ...
        return (
            len(chuukese_word) >= 2
            and len(english_translation) >= 3
            and len(chuukese_word) <= 40
            and len(english_translation) <= 250
            and chuukese_word.replace("-", "").replace("*", "").replace(" ", "").isalpha()
            and chuukese_word not in ["v", "n", "adj", "adv", "prep"]
        )  # Exclude grammatical markers
    ...
    def _normalize_grammar(self, grammar: str) -> str | None:
    ...
    def _merge_duplicate_entry(self, new_entry: dict, page_id: str, filename: str, page_number: int):
        ...
        try:
            ...
            existing = self.dictionary_collection.find_one(
                {"chuukese_word": new_entry["chuukese_word"], "english_translation": new_entry["english_translation"]}
            )
            ...
            # Apply update
            self.dictionary_collection.update_one({"_id": existing["_id"]}, update_operation)
    ...
    def reprocess_page(self, publication_id: str, filename: str, page_number: int = 1) -> dict:
        ...
        existing_page = self.pages_collection.find_one(
            {"publication_id": publication_id, "filename": filename, "page_number": page_number}
        )
        ...
        self.pages_collection.update_one(
            ...
            {"$set": {"reprocessed_date": datetime.now(timezone.utc), "entries_extracted": len(entries)}},
        )
    ...
    def search_word(
        self, word: str, limit: int = 10, include_related: bool = True, exact_match: bool = False
    ) -> list[dict]:
        """
        ...
        if exact_match:
            ...
            query = {
                "$and": [
                    ...
                    {"$or": [{"search_direction": {"$exists": False}}, {"search_direction": {"$ne": "en_to_chk"}}]},
                ]
            }
        ...
        for result in results:
            ...
            if unique_key not in unique_entries or result.get("confidence", 0) > unique_entries[unique_key].get(
                "confidence", 0
            ):
                # Normalize field names for consistency - ensure chuukese_word is populated
                if not result.get("chuukese_word"):
                    result["chuukese_word"] = (
                        result.get("chuukese_sentence") or result.get("chuukese_phrase") or result.get("chuukese") or ""
                    )
        ...
        for result in results:
            ...
            if "created_date" in result and result["created_date"]:
                result["created_date"] = (
                    ...
                    if hasattr(result["created_date"], "isoformat")
                    else str(result["created_date"])
                )
            if "processed_date" in result and result["processed_date"]:
                result["processed_date"] = (
                    result["processed_date"].isoformat()
                    if hasattr(result["processed_date"], "isoformat")
                    else str(result["processed_date"])
                )
    ...
    def get_stats(self) -> dict:
        ...
        try:
            ...
            chuukese_words_pipeline = [{"$group": {"_id": "$chuukese_word"}}, {"$count": "total"}]
            ...
            english_words_pipeline = [{"$group": {"_id": "$english_translation"}}, {"$count": "total"}]
            ...
            words_count = self.dictionary_collection.count_documents({"chuukese_word": {"$not": {"$regex": " "}}})
            phrases_count = self.dictionary_collection.count_documents({"chuukese_word": {"$regex": " "}})

            # Count sentences (type = 'sentence') — also check phrases_collection
            sentences_count = self.dictionary_collection.count_documents({"type": "sentence"})
            try:
                sentences_count += self.phrases_collection.count_documents({"type": "sentence"})
            except Exception:
                pass
            ...
            if sample_entry and "created_date" in sample_entry:
                last_updated = (
                    sample_entry["created_date"].isoformat()
                    if hasattr(sample_entry["created_date"], "isoformat")
                    else str(sample_entry["created_date"])
                )
    ...
    def get_recent_entries(self, limit: int = 20) -> list[dict]:
        ...
        results = list(self.dictionary_collection.find({}).sort("created_date", -1).limit(limit))
    ...
    def get_pages_summary(self) -> list[dict]:
        ...
        results = list(self.pages_collection.find({}).sort("processed_date", -1))
    ...
    def get_entries_paginated(self, page: int = 1, per_page: int = 50, search: str = "") -> dict:
        ...
        results = list(self.dictionary_collection.find(query).sort("created_date", -1).skip(skip).limit(per_page))
    ...
    def add_dictionary_from_csv(
        self, publication_id: str, filename: str, csv_content: str, confidence_score: int = 100
    ) -> tuple[str, int]:
        """
    ...
    def _extract_entries_from_csv(
        self, csv_content: str, page_id: str, publication_id: str, filename: str, confidence_score: int = 100
    ) -> list[dict]:
        """
        ...
        try:
            ...
            if not header:
                print("⚠️ No header found in CSV")
                return []
            ...
            for row_num, row in enumerate(csv_reader, 2):
                ...
                try:
                    if is_standard_format:
                        ...
                        entry_type = (
                            row[col_map.get("type", 2)].strip().lower() if len(row) > col_map.get("type", 2) else "word"
                        )
                        grammar = (
                            row[col_map.get("grammar", 3)].strip().lower()
                            if len(row) > col_map.get("grammar", 3)
                            else ""
                        )
                        search_direction = (
                            row[col_map.get("search_direction", 4)].strip()
                            if len(row) > col_map.get("search_direction", 4)
                            else "chk_to_en"
                        )
                        definition = (
                            row[col_map.get("definition_notes", 5)].strip()
                            if len(row) > col_map.get("definition_notes", 5)
                            else ""
                        )
                        ...
                        if entry_type == "word" or entry_type in grammar_mapping or "+" in entry_type:
                            ...
                            word_entry = {
                                ...
                                "grammar": self._normalize_grammar(
                                    mapped_grammar
                                ),  # noun, verb, adjective, etc. (full form)
                        ...
                        word_type = (
                            self._extract_word_type(part_of_speech, english_translation) if part_of_speech else None
                        )
        ...
        print(
            f"✅ Extracted: {len(words)} words, {len(phrases)} phrases, {len(sentences)} sentences, {len(paragraphs)} paragraphs"
        )
        ...
        if words:
            self._batch_insert(self.dictionary_collection, words, batch_size, "words")
            all_entries.extend(words)

        # Insert phrases into phrases_collection
        if phrases:
            self._batch_insert(self.phrases_collection, phrases, batch_size, "phrases")
            all_entries.extend(phrases)

        # Insert sentences into phrases_collection (sentences are stored with phrases)
        if sentences:
            self._batch_insert(self.phrases_collection, sentences, batch_size, "sentences")
            all_entries.extend(sentences)

        # Insert paragraphs into paragraphs_collection
        if paragraphs:
            self._batch_insert(self.paragraphs_collection, paragraphs, batch_size, "paragraphs")
    ...
    def _batch_insert(self, collection, items, batch_size, item_type):
        ...
        print(
            f"📦 Starting batch insert: {len(items)} {item_type} into '{collection_name}' ({total_batches} batches of {batch_size})"
        )
            ...
            while retries > 0:
                try:
                    ...
                    if "16500" in error_str or "RetryAfterMs" in error_str:
                        ...
                        print(
                            f"  ⏳ [{collection_name}] Rate limited on batch {batch_num}, waiting {delay:.1f}s ({retries} retries left)"
                        )
                        time.sleep(delay)
                    else:
    ...
    def add_word(self, chuukese: str, english_translation: str, grammar: str = None, **kwargs) -> str | None:
        """
    ...
    def update_word(self, word_id: str, **updates) -> bool:
        ...
        try:
            if self.db_type == "cosmos":
                ...
                self.words_collection.update_one({"id": word_id}, {"$set": updates})
            return True
        except Exception as e:
    ...
    def add_phrase(self, chuukese_phrase: str, english_translation: str, source: str = None, **kwargs) -> str | None:
        """
    ...
    def search_phrases(self, query: str, limit: int = 50) -> list[dict]:
        ...
        try:
            if self.db_type == "cosmos":
                ...
                """
                results = list(
                    self.phrases_collection.query_items(
                        query=sql_query,
    ...
    def update_phrase(self, phrase_id: str, **updates) -> bool:
        ...
        try:
            if self.db_type == "cosmos":
                ...
                self.phrases_collection.update_one({"id": phrase_id}, {"$set": updates})
            return True
        except Exception as e:
    ...
    def add_paragraph(
        self, chuukese_paragraph: str, english_paragraph: str, source: str = None, **kwargs
    ) -> str | None:
        """
    ...
    def search_paragraphs(self, query: str, limit: int = 20) -> list[dict]:
        ...
        try:
            if self.db_type == "cosmos":
                ...
                """
                results = list(
                    self.paragraphs_collection.query_items(
                        query=sql_query,
                        parameters=[{"name": "@query", "value": query}],
                        enable_cross_partition_query=True,
                        max_item_count=limit,
                ...
                # MongoDB query
                results = list(
                    self.paragraphs_collection.find(
                        {
                            "$or": [
    ...
    def update_paragraph(self, paragraph_id: str, **updates) -> bool:
        ...
        try:
            if self.db_type == "cosmos":
                ...
                self.paragraphs_collection.update_one({"id": paragraph_id}, {"$set": updates})
            return True
        except Exception as e:
