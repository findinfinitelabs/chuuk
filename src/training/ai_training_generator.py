...
class AITrainingDataGenerator:
    ...
    def __init__(self, min_confidence: float = 0.7):
        ...
        self.word_patterns = {
            ...
            "mixed_language": re.compile(
                r"[a-zA-Z\s\-\'].*[áéíóúàèìòùāēīōūâêîôû]|[áéíóúàèìòùāēīōūâêîôû].*[a-zA-Z\s\-\']"
            ),
        }
    ...
    def generate_comprehensive_training_data(
        self, parsed_doc, target_count: int = 10000
                                ...
                                for source, target in variations:
                                    pairs.append(
                                        TrainingExample(
                                            ...
                                            metadata={"original_text": text, "extraction_method": "pattern_split"},
                                        )
                                    )
                                ...
                                if self._is_valid_word(word) and len(definition) > 5:
                                    examples.append(
                                        TrainingExample(
                                            ...
                                            metadata={"definition_word": def_word, "context": context_text[:200]},
                                        )
                                    )
                ...
                for node in sample_nodes:
                    ...
                    examples.append(
                        TrainingExample(
                            ...
                            metadata={"structure_level": level, "formatting": str(node.formatting)},
                        )
                    )
                    ...
                    if node.children:
                        ...
                        examples.append(
                            TrainingExample(
                                ...
                                metadata={"parent_text": node.text, "child_count": len(node.children)},
                            )
                        )
    ...
    def _generate_classification_tasks(self, parsed_doc) -> list[TrainingExample]:
        ...
        content_types = {"definition": [], "example": [], "heading": [], "list_item": [], "explanation": []}
            ...
            for text, node in items[:20]:  # Limit per category
                examples.append(
                    TrainingExample(
                        ...
                        metadata={"true_type": content_type, "structure_level": node.level},
                    )
                )
    ...
    def _filter_and_balance_data(
        self, training_data: dict[str, list[TrainingExample]], target_count: int
    ) -> dict[str, list[TrainingExample]]:
    ...
    def export_training_data(
        self, training_data: dict[str, list[TrainingExample]], output_dir: str, format_type: str = "jsonl"
    ) -> dict[str, str]:
        """
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    for ex in examples:
                        ollama_obj = {
                            ...
                            "system": "You are a helpful assistant specialized in Chuukese language and dictionary content.",
                        }
...
def main():
    ...
    parser.add_argument(
        "--format", choices=["jsonl", "csv", "huggingface", "ollama"], default="jsonl", help="Output format"
    )
