...
class EnhancedOCRProcessor(OCRProcessor):
    ...
    def process_large_document(
        self, file_path: str, output_dir: str = None, generate_training_data: bool = True, chunk_for_ai: bool = True
    ) -> dict[str, any]:
        """
        ...
        results = self.large_doc_processor.process_large_document(file_path, output_dir)
    ...
    def process_with_intelligent_chunking(
        self, file_path: str, chunk_type: str = "semantic", max_chunk_size: int = 512
    ) -> dict[str, any]:
        """
    ...
    def extract_training_examples(self, file_path: str, min_examples: int = 1000) -> dict[str, any]:
        """
        ...
        exported_files = self.training_generator.export_training_data(training_data, output_dir, format_type="ollama")
                ...
                if sep in line:
                    ...
                    if len(parts) == 2:
                        ...
                        if len(source) > 1 and len(target) > 2:
                            examples.append(
                                {"input": f"What does '{source}' mean?", "output": target, "source": "ocr_extraction"}
                            )
                    break
