"""
OCR processing module supporting Tesseract, Google Vision API, and DOCX
"""
...
class OCRProcessor:
    """Handles OCR processing using Tesseract, Google Vision API, and DOCX text extraction"""
    ...
    def __init__(self, use_google_vision: bool = False):
        ...
        if self.use_google_vision:
            ...
            if api_key:
                print("🔑 Google Vision API key found - will use REST API")
    ...
    def process_image_google_vision(self, image_path: str) -> str:
        ...
        try:
            ...
            data = {"requests": [{"image": {"content": encoded_image}, "features": [{"type": "TEXT_DETECTION"}]}]}
    ...
    def process_docx_to_text(self, docx_path: str) -> dict:
        """
        ...
        """
        if not DOCX_SUPPORT:
            return {"error": "DOCX processing not available. Install python-docx.", "text": "", "type": "docx"}
        ...
        try:
            ...
            if current_page_text:
                pages_data.append(
                    {"page_number": current_page, "text": "\n".join(current_page_text), "total_pages": None}
                )
            ...
            return {"error": f"Error processing DOCX: {str(e)}", "text": "", "type": "docx"}
    ...
    def process_image(self, file_path: str, lang: str = "eng") -> dict:
        ...
        # Handle PDF files - return page-by-page results
            ...
            try:
                pdf_images = self.process_pdf_to_images(file_path)
                pages_data = []

                for i, image in enumerate(pdf_images):
                    page_result = {"page_number": i + 1, "total_pages": len(pdf_images)}
                    ...
                    if self.use_google_vision:
                        ...
                        temp_image_path = f"{file_path}_temp_page_{i+1}.png"
        ...
        # Handle regular image files
            ...
            # Try Tesseract
            tesseract_text = self.process_image_tesseract(file_path, lang)
            results["tesseract"] = tesseract_text

            # Try Google Vision if enabled
            if self.use_google_vision:
                google_text = self.process_image_google_vision(file_path)
