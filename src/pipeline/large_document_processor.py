...
class LargeDocumentProcessor:
    ...
    def process_large_document(self, input_file: str, output_dir: str) -> dict[str, any]:
        ...
        logger.info("🚀 Starting large document processing pipeline")
        ...
        try:
            ...
            if self.config.generate_training_data:
                ...
                training_data = self.training_generator.generate_comprehensive_training_data(
                    parsed_doc, target_count=self.config.target_training_examples
                )
                ...
                for format_type in self.config.output_formats:
                    format_files = self.training_generator.export_training_data(
                        training_data, str(training_dir), format_type=format_type
                    )
    ...
    def process_multiple_documents(self, input_files: list[str], output_base_dir: str) -> dict[str, any]:
        ...
        results = {"total_documents": len(input_files), "processing_start": time.time(), "documents": {}}
        ...
        with ThreadPoolExecutor(max_workers=self.config.parallel_workers) as executor:
            futures = {executor.submit(process_single_doc, file_path): file_path for file_path in input_files}
                ...
                if sep in text and not self._is_page_marker(text):
                    ...
                    if len(parts) == 2:
                        ...
                        if (
                            len(chuukese) > 1
                            and len(english) > 2
                            and len(chuukese.split()) <= 5
                            and not chuukese.isdigit()
                        ):
    ...
    def _is_page_marker(self, text: str) -> bool:
        ...
        patterns = [r"^page\s+\d+", r"^\d+$", r"^p\.\s*\d+", r"chapter\s+\d+", r"section\s+\d+"]
        ...
        try:
            ...
            for entry in entries:
                ...
                existing = self.db.dictionary_collection.find_one(
                    {"chuukese_word": db_entry["chuukese_word"], "english_translation": db_entry["english_translation"]}
                )
    ...
    def _generate_processing_report(self, results: dict, parsed_doc: ParsedDocument, output_file: str):
        ...
        report = {
            ...
            "document_analysis": {
                ...
                "structure_levels": len({node.level for node in parsed_doc.structure_tree}),
        ...
        if "training_data" in results["stages"]:
            ...
            if total_examples < 1000:
                recommendations.append(
                    "Low training data yield - consider improving dictionary entry detection patterns"
                )
...
def main():
    ...
    parser.add_argument(
        "--formats",
        nargs="+",
        choices=["jsonl", "csv", "huggingface", "ollama"],
        default=["jsonl", "ollama"],
        help="Output formats",
    )
    ...
    try:
        if args.batch and input_path.is_dir():
            ...
            input_files = [
                str(f) for f in input_path.rglob("*") if f.suffix.lower() in supported_extensions and f.is_file()
            ]
            ...
            processor.process_multiple_documents(input_files, args.output_dir)
            ...
            processor.process_large_document(str(input_path), args.output_dir)
