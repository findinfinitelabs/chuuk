...
class IntelligentTextChunker:
    ...
    def chunk_document(self, text: str, chunk_type: ChunkType = ChunkType.SEMANTIC) -> list[TextChunk]:
        ...
        logger.info(
            f"✅ Created {len(chunks)} chunks (avg size: {sum(len(c) for c in chunks) / len(chunks):.0f} chars)"
        )
    ...
    def _split_by_semantic_boundaries(self, text: str) -> list[str]:
        ...
        for i, sentence in enumerate(sentences):
            ...
            has_topic_change = any(
                indicator in sentence_lower for indicator in self.semantic_indicators["topic_change"]
            )

            # Check for conclusion indicators (end of section)
            has_conclusion = any(indicator in sentence_lower for indicator in self.semantic_indicators["conclusion"])
    ...
    def _optimize_chunks(self, chunks: list[TextChunk]) -> list[TextChunk]:
        ...
        while i < len(chunks):
            ...
            if (
                ...
                and i + 1 < len(chunks)
                and len(current_chunk) + len(chunks[i + 1]) <= self.max_chunk_size
            ):
    ...
    def get_chunk_statistics(self, chunks: list[TextChunk]) -> dict[str, any]:
        ...
        stats = {
            ...
            "chunk_types": {
                chunk.chunk_type.value: sum(1 for c in chunks if c.chunk_type == chunk.chunk_type) for chunk in chunks
            },
            "overlap_stats": {
...
def main():
    ...
    parser.add_argument(
        "--chunk-type",
        choices=["semantic", "structural", "fixed_size", "sliding_window"],
        default="semantic",
