...
print("\n=== shelf (English) ===")
results = collection.find({"english": {"$regex": "shelf", "$options": "i"}}).limit(10)
for r in results:
    ch = r.get("chuukese", "")
    en = r.get("english", "")
    print(f"  {ch} = {en}")
...
print("\n=== book (English) ===")
results = collection.find({"english": {"$regex": "^book$", "$options": "i"}}).limit(10)
for r in results:
    ch = r.get("chuukese", "")
    en = r.get("english", "")
    print(f"  {ch} = {en}")
...
for clf in classifiers:
    results = collection.find({"chuukese": {"$regex": f"^{clf}$", "$options": "i"}}).limit(5)
    for r in results:
        ch = r.get("chuukese", "")
        en = r.get("english", "")
        print(f"  {ch} = {en}")
