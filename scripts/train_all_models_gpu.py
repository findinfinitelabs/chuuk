...
class ComprehensiveModelTrainer:
    ...
    def _load_from_json(self):
        ...
        with open(self.json_export, encoding="utf-8") as f:
            entries = json.load(f)

        self.log(f"✅ Loaded {len(entries)} entries")

        training_data = {"words": [], "phrases": [], "sentences": [], "translation_game": []}
    ...
    def _load_from_database(self):
        ...
        words = list(self.db.dictionary_collection.find({"search_direction": {"$ne": "en_to_chk"}}))
        for word in words:
        ...
        for phrase in phrases:
            chk = (
                phrase.get("chuukese_sentence") or phrase.get("chuukese_phrase") or phrase.get("chuukese", "")
            ).strip()
            ...
            if chk and eng and len(chk) > 2 and len(eng) > 3:
                ...
                if source_type == "translation_game":
                    ...
                    training_data["sentences"].append({"chuukese": chk, "english": eng, "type": "sentence"})
                else:
                    training_data["phrases"].append({"chuukese": chk, "english": eng, "type": "phrase"})
    ...
    def train_ollama_model(self, training_data):
        ...
        try:
            ...
            for item in all_examples:
                ...
                prompts = [
                    {"input": f"Translate to English: {item['chuukese']}", "output": item["english"]},
                    {"input": f"What does '{item['chuukese']}' mean in English?", "output": item["english"]},
                    {"input": f"Translate to Chuukese: {item['english']}", "output": item["chuukese"]},
                ]
