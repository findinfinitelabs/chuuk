...
for key, term in classifiers.items():
    print(f"\n{key.upper().replace('_', ' ')}:")
    results = list(db.numbers_collection.find({
        '$or': [
        ...
        'number_value': {'$lte': 10}
    }).sort('number_value', 1).limit(10))
    
    if results:
        for r in results:
            print(f"  {r.get('number_value', 'N/A')}: {r.get('chuukese_word', 'N/A')} - {r.get('english_translation', '')}")
    else:
...
for r in all_nums[:20]:
    print(f"  {r.get('number_value', 'N/A')}: {r.get('chuukese_word', 'N/A')} - Type: {r.get('type', 'N/A')} - Grammar: {r.get('grammar', 'N/A')}/{r.get('grammar_modifier', 'N/A')}")
