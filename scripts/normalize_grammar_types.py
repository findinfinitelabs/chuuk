        ...
        if count > 0:
            ...
            if not dry_run:
                ...
                for entry in entries:
                    try:
                        if new_grammar is None:
                            db.dictionary_collection.update_one({"_id": entry["_id"]}, {"$unset": {"grammar": ""}})
                        else:
                            db.dictionary_collection.update_one(
                                {"_id": entry["_id"]}, {"$set": {"grammar": new_grammar}}
                            )
                        updated += 1
                    except Exception as e:
                        if "16500" in str(e):
