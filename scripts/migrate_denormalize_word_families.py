...
def migrate_denormalize_families():
    ...
    for entry in all_entries:
        ...
        word_families[base_word].append(
            {"word": word, "translation": translation, "word_type": entry.get("word_type", ""), "entry_id": entry_id}
        )
    ...
    for entry in all_entries:
        ...
        related_words = [
            {"word": member["word"], "translation": member["translation"], "word_type": member["word_type"]}
            for member in family_members
            if member["word"] != word  # Exclude self
        ]

        # Update the entry
        update_data = {"related_words": related_words, "word_family_size": len(family_members), "base_word": base_word}

        # Only update if there are changes
        if entry.get("related_words") != related_words:
            dictionary_collection.update_one({"_id": entry["_id"]}, {"$set": update_data})
            updated_count += 1
