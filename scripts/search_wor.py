...
print("=== wor pattern ===")
results = list(collection.find({"chuukese": {"$regex": "wor", "$options": "i"}}).limit(20))
for r in results:
...
print("\n=== eew/ew/eu (one) ===")
results = list(collection.find({"chuukese": {"$regex": "^e(u|w|ew)", "$options": "i"}}).limit(20))
for r in results:
    print(f"  {r.get('chuukese', '')} = {r.get('english', '')}")

print("\n=== won/woon (on) ===")
results = list(collection.find({"chuukese": {"$regex": "^woo?n", "$options": "i"}}).limit(10))
for r in results:
    print(f"  {r.get('chuukese', '')} = {r.get('english', '')}")
