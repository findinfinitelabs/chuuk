...
for entry in entries:
    try:
        ...
        db.dictionary_collection.update_one(
            {"_id": entry["_id"]}, {"$set": {"sort_order": sort_order, "number_type": number_type}}
        )
        updated += 1
...
# Verify the update
print("\n=== Verification ===")
sample = list(db.dictionary_collection.find({"sort_order": {"$exists": True}}).sort("sort_order", 1).limit(15))
