...
for entry in base_entries:
    try:
        db.dictionary_collection.update_one({"_id": entry["_id"]}, {"$set": {"number_type": "base_cardinal"}})
        updated += 1
        if updated % 100 == 0:
...
for entry in cardinal_entries:
    try:
        db.dictionary_collection.update_one({"_id": entry["_id"]}, {"$set": {"number_type": "cardinal"}})
        updated += 1
        if updated % 100 == 0:
...
for s in base_sample:
    print(
        f"  {s.get('chuukese_word'):25} -> {s.get('english_translation'):5} | sort: {s.get('sort_order')} | type: {s.get('number_type')}"
    )
