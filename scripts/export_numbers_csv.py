...
def export_numbers_to_csv():
    ...
    ordinal_query = {"definition": {"$regex": "ordinal number", "$options": "i"}}

    # Query for cardinal numbers - look for "cardinal number" in definition
    cardinal_query = {"definition": {"$regex": "cardinal number", "$options": "i"}}
...
def write_csv(filepath, entries, dict_entries):
    ...
    with open(filepath, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["chuukese_word", "english_translation", "definition", "grammar", "type", "source", "collection"]
        )
