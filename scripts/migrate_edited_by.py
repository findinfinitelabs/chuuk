...
def update_batch(coll, name):
    ...
    ids = _retry(
        lambda: [doc["_id"] for doc in coll.find({"edited_by": {"$exists": False}}, {"_id": 1}).limit(BATCH)],
        f"find-{name}",
    )
    if not ids:
        return 0

    result = _retry(lambda: coll.update_many({"_id": {"$in": ids}}, {"$set": {"edited_by": "MI"}}), f"update-{name}")
...
def migrate():
    ...
    for name, coll in collections:
        ...
        missing = _retry(lambda: coll.count_documents({"edited_by": {"$exists": False}}), f"count-{name}")
