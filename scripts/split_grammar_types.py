...
def split_grammar(original_grammar: str | None) -> tuple[str | None, str | None]:
    """
...
def migrate_grammar_types(dry_run: bool = True, max_entries: int = None, skip_confirm: bool = False):
    ...
    for entry in entries:
        ...
        if original not in grammar_stats:
            grammar_stats[original] = {"count": 0, "core": core, "modifier": modifier, "sample_ids": []}
    ...
    for i, entry in enumerate(entries, 1):
        ...
        # Update the entry
        update_data = {"grammar": core, "grammar_modifier": modifier}
        ...
        while retry_count <= max_retries:
            try:
                db.dictionary_collection.update_one({"_id": entry["_id"]}, {"$set": update_data})
                success_count += 1
        ...
        if i % 50 == 0:
            print(
                f"  Processed {i:,}/{len(entries):,} ({i*100//len(entries)}%) - "
...
Examples:
  ...
  ./venv/bin/python scripts/split_grammar_types.py --samples
    ...
    parser.add_argument("--samples", action="store_true", help="Show sample entries with split grammar")
